"use client";

import { updateProfile } from "@/lib/action";
import { User } from "@prisma/client";
import Image from "next/image";
import { useActionState, useState } from "react";
import { CldUploadWidget } from "next-cloudinary";
import { useRouter } from "next/navigation";
import UpdateButton from "./UpdateButton";

function UpdateUser({ user }: { user: User }) {
  const [open, setOpen] = useState(false);
  const [coverImage, setCoverImage] = useState<any>();
  const [formState, formAction] = useActionState(updateProfile, {
    success: false,
    error: false,
  });

  const router = useRouter();
  const handleClose = () => {
    setOpen(false);
    formState.success && router.refresh();
  };

  return (
    <div>
      <span
        className="cursor-pointer text-xs text-blue-500"
        onClick={() => setOpen(true)}
      >
        Update
      </span>

      {open && (
        <div className="absolute left-0 top-0 z-50 flex h-screen w-screen items-center justify-center bg-black bg-opacity-65">
          <form
            action={(formData) =>
              formAction({
                formdata: formData,
                coverImageUrl: coverImage?.secure_url || "",
              })
            }
            className="relative flex w-full flex-col gap-2 rounded-lg bg-white p-12 shadow-md md:w-1/2 xl:w-1/3"
          >
            {/* Title  */}
            <h1>Update Profile</h1>
            <div className="tex-xs mt-4 text-gray-500">
              Use the navbar profile to change the avatar or username.
            </div>

            <CldUploadWidget
              uploadPreset="funsocial"
              onSuccess={(result) => setCoverImage(result.info)}
            >
              {({ open }) => {
                return (
                  <div
                    className="my-4 flex flex-col gap-4"
                    onClick={() => open()}
                  >
                    <label htmlFor="">Cover Picture</label>
                    <div className="flex cursor-pointer items-center gap-2">
                      <Image
                        src={user.cover || "/noCover.png"}
                        alt=""
                        width={480}
                        height={32}
                        className="h-8 w-12 rounded-md object-cover"
                      />
                      <span className="text-xs text-gray-600 underline">
                        Change
                      </span>
                    </div>
                  </div>
                );
              }}
            </CldUploadWidget>

            <div className="flex flex-wrap justify-between gap-2 xl:gap-4">
              {/* INPUT */}

              <div className="flex flex-col gap-4">
                <label htmlFor="" className="text-xs text-gray-500">
                  First Name
                </label>
                <input
                  name="name"
                  id=""
                  className="rounded-md p-[12px] text-sm ring-1 ring-gray-300"
                  placeholder={user.username || "John"}
                />
              </div>
              {/* INPUT */}

              <div className="flex flex-col gap-4">
                <label htmlFor="" className="text-xs text-gray-500">
                  Surname
                </label>
                <input
                  name="surname"
                  id=""
                  className="rounded-md p-[12px] text-sm ring-1 ring-gray-300"
                  placeholder={user.surname || "Doe"}
                />
              </div>

              {/* INPUT */}

              <div className="flex flex-col gap-4">
                <label htmlFor="" className="text-xs text-gray-500">
                  Description
                </label>
                <input
                  name="description"
                  className="rounded-md p-[12px] text-sm ring-1 ring-gray-300"
                  id=""
                  placeholder={user.description || "happy..."}
                />
              </div>
              {/* INPUT */}

              <div className="flex flex-col gap-4">
                <label htmlFor="" className="text-xs text-gray-500">
                  City
                </label>
                <input
                  name="city"
                  id=""
                  className="rounded-md p-[12px] text-sm ring-1 ring-gray-300"
                  placeholder={user.city || "Bombay"}
                />
              </div>

              {/* INPUT */}

              <div className="flex flex-col gap-4">
                <label htmlFor="" className="text-xs text-gray-500">
                  School
                </label>
                <input
                  name="school"
                  id=""
                  className="rounded-md p-[12px] text-sm ring-1 ring-gray-300"
                  placeholder={user.school || "School"}
                />
              </div>

              {/* INPUT */}

              <div className="flex flex-col gap-4">
                <label htmlFor="" className="text-xs text-gray-500">
                  Work
                </label>
                <input
                  name="work"
                  id=""
                  className="rounded-md p-[12px] text-sm ring-1 ring-gray-300"
                  placeholder={user.work || "Apple"}
                />
              </div>

              {/* INPUT */}

              <div className="flex flex-col gap-4">
                <label htmlFor="" className="text-xs text-gray-500">
                  Website
                </label>
                <input
                  name="website"
                  id=""
                  className="rounded-md p-[12px] text-sm ring-1 ring-gray-300"
                  placeholder={user.website || "google.com"}
                />
              </div>
            </div>

          <UpdateButton/>

            {formState.success && (
              <span className="text-xs text-green-500">
                Profile has been successfully updated!
              </span>
            )}
            {formState.error && (
              <span className="text-xs text-red-500">
                Failed to upddate profile!
              </span>
            )}
            <div
              className="absolute right-5 top-3 cursor-pointer text-xl"
              onClick={handleClose}
            >
              X
            </div>
          </form>
        </div>
      )}
    </div>
  );
}

export default UpdateUser;
